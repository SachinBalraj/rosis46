  ...
  return (
    ...
    >
      <div className="animate-pulse space-y-4">
        ...
        <div className="h-10 w-72 max-w-full bg-line" />
        <div className="h-4 w-96 max-w-full rounded-full bg-line" />
      </div>
      <div className="animate-pulse grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {Array.from({ length: 8 }).map((_, i) => (
          <div key={i} className="h-80 border border-line bg-carbon-soft" />
        ))}
      </div>
    </div>
  );
